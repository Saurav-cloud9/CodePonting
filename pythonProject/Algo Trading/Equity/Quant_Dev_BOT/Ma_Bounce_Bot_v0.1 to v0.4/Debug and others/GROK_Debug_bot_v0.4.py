...
ACCESS_TOKEN = "eyJ0eXAiOiJKV1QiLCJrZXlfaWQiOiJza192MS4wIiwiYWxnIjoiSFMyNTYifQ.eyJzdWIiOiJFRTY4MTkiLCJqdGkiOiI2OTQ5MGY3ZmUzYTIyMjQ4YWU4NzRlZGQiLCJpc011bHRpQ2xpZW50IjpmYWxzZSwiaXNQbHVzUGxhbiI6ZmFsc2UsImlhdCI6MTc2NjM5NTc3NSwiaXNzIjoidWRhcGktZ2F0ZXdheS1zZXJ2aWNlIiwiZXhwIjoxNzY2NDQwODAwfQ.WcbrB-HfjU8v_T2GWG1h9uVLlahQsxt6BjIKuXy1K1c"
...
def get_current_strategy():
    ...
    market_close = datetime.strptime("15:30", "%H:%M").time()  # 3:15 PM
...
def get_live_price(instrument):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            # Key is shortened: e.g., "NSE_EQ:YESBANK" for YESBANK
            # Extract the symbol part from your instrument key
            symbol_part = instrument.split('|')[-1]  # e.g., "INE528G01035" → but actually use the known mapping or find the key
            # Better: Since we know the response key is "NSE_EQ:<SYMBOL>"
            # Get the first (only) data key dynamically
            if 'data' in data and data['data']:
                first_key = list(data['data'].keys())[0]  # e.g., "NSE_EQ:YESBANK"
...
def get_historical_candles(instrument, num_minutes=100):
    try:
        ...
        params = {"instrument_key": instrument}   # ← THIS WAS MISSING!
        ...
        if response.status_code == 200:
            ...
            print(f"Error getting candles: {response.status_code} - {response.text}")
            return []
        ...
        try:
            ...
            for symbol, instrument in WATCHLIST.items():
                ...
                if has_signal:
                    live_price = get_live_price(instrument)
                    if live_price:
                        place_order(symbol, instrument, live_price, strategy)
