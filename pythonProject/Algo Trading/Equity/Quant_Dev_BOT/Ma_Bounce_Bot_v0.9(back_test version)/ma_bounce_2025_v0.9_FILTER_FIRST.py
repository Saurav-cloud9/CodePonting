...
└─────────────────────────────────────────────────────────────┘
              ↓
┌─────────────────────────────────────────────────────────────┐
│  STEP 3: TOUCH & VOLUME CHECK (Before bounce detection!)   │
├─────────────────────────────────────────────────────────────┤
│ (Only check candles that passed uptrend filter)            │
│                                                              │
│ ✅ TOUCH: IF low[i] <= ma20[i]                              │
│                                                              │
│ ✅ VOLUME CHECK (Before expensive bounce detection):        │
│    avg_volume = avg(volume[i-19 to i-1])                   │
│    IF volume[i] > avg_volume × 1.2:                         │
│       ✅ VOLUME CONFIRMED - Count for stats                 │
│       Proceed to bounce detection                           │
│    ELSE:                                                     │
│       ❌ Skip (weak volume)                                 │
└─────────────────────────────────────────────────────────────┘
              ↓
┌─────────────────────────────────────────────────────────────┐
│  STEP 4: BOUNCE DETECTION (Only on good volume touches)    │
├─────────────────────────────────────────────────────────────┤
│ (Only check after touch + volume confirmed)                │
│                                                              │
...
│       IF close[j] > ma20[i]:                                │
│          ✅ DELAYED BOUNCE at candle j - Count for stats    │
...
STOCK_NAME = sys.argv[1] if len(sys.argv) > 1 else 'TATASTEEL'
...
def run_backtest(df):
    ...
    for i in range(len(df)):
        ...
        # ✅ FIX: Count volume_confirmed HERE for all filters that passed uptrend
        for filter_name, passed in filters_passed.items():
            if passed:
        ...
        for filter_name, passed in filters_passed.items():
            ...
            # ✅ FIX: Only count confirmed bounces (touch + volume + bounce confirmation)
    ...
    print("-"*90)
    print("(Uptrend: price > filter MAs | Volume OK: touches with good volume | Bounces: confirmed bounces)")
    print("-"*90)
    for filter_name in FILTER_NAMES:
        stats = filter_stats[filter_name]
        print(f"{filter_name:<16} | Uptrend: {stats['uptrend']:>5} | Volume OK: {stats['volume_confirmed']:>4} | Bounces: {stats['bounces']:>4}")
