...
import os
import warnings
import yfinance as yf  # For NIFTY context only
...
def run_ma_bounce(df, stock_name):
    ...
    # Calculate profit efficiency (ROI per share)
...
def fetch_nifty_context(from_date, to_date):
    """Fetch NIFTY 50 context using Yahoo Finance (Upstox doesn't support index data)"""
    print("\n" + "="*90)
    print("FETCHING NIFTY 50 CONTEXT (via Yahoo Finance)")
    print("="*90)
    
    try:
        # Need 200 trading days for MA200 calculation
        # 200 trading days = ~280 calendar days (accounting for weekends/holidays)
        historical_start = from_date - timedelta(days=300)  # Extra buffer for safety
        
        # Download NIFTY data from Yahoo Finance
        # ✅ Fixed: Flatten MultiIndex columns by accessing as single ticker
        nifty = yf.download('^NSEI', start=historical_start, end=to_date + timedelta(days=1),
        ...
        if len(nifty) < 200:
            print(f"✗ Insufficient data: Got {len(nifty)} days, need 200 for MA200")
            return None
        
        # ✅ Fixed: Handle MultiIndex columns - flatten if needed
        ...
        # Get data for the backtest month only - proper pandas date filtering
        month_data = nifty.loc[from_date.strftime('%Y-%m-%d'):to_date.strftime('%Y-%m-%d')]

        if len(month_data) == 0:
            print("✗ No NIFTY data for selected month")
            return None
        
        # Month metrics - ✅ Fixed: Ensure we get scalar values
        ...
        # Determine market regime using last day's MA values
        last_row = nifty.iloc[-1]
        last_close = float(last_row['Close'])
        last_ma50 = float(last_row['MA50']) if pd.notna(last_row['MA50']) else None
        last_ma200 = float(last_row['MA200']) if pd.notna(last_row['MA200']) else None

        # ✅ Fixed: Use proper None checks instead of pd.notna
        ...
        print(f"✓ Data points fetched: {len(nifty)} days")
...
def export_to_excel(all_results, nifty_context, month_name, folder):
    """Export results to Excel with 5 sheets (TOC + 4 data sheets)"""
    ...
    with pd.ExcelWriter(filename, engine='openpyxl') as writer:
        ...
        # ═══════════════════════════════════════════════════════════════
        # Sheet 0: TABLE OF CONTENTS (Navigation Hub) - ✅ Added
        # ═══════════════════════════════════════════════════════════════
        toc_data = [
            ['═══════════════════════════════════════════════════════'],
            [f'MA BOUNCE BACKTEST - {month_name}'],
            ['═══════════════════════════════════════════════════════'],
            [''],
            ['📊 QUICK NAVIGATION - Click the links below:'],
            [''],
            ['🔗 Summary Sheet', '→ View all stocks summary with NIFTY context'],
            ['🔗 Winners Sheet', '→ Stocks with >55% win rate'],
            ['🔗 Losers Sheet', '→ Stocks with ≤55% win rate'],
            ['🔗 Pattern Analysis', '→ Grouped by price vs MA patterns'],
            [''],
            [''],
            ['💡 TIP: Click sheet tabs at bottom to navigate manually'],
            ['💡 TIP: Each sheet has a "Back to TOC" link at top']
        ]

        df_toc = pd.DataFrame(toc_data)
        df_toc.to_excel(writer, sheet_name='📑 TOC', index=False, header=False)
        ...
        for result in all_results:
            summary_data.append({
                ...
                'Efficiency (%)': round(result['profit_efficiency'], 2),
        ...
        # Add NIFTY context header at TOP
        header_rows = []
        if nifty_context:
            header_rows.append(['═══ MARKET CONTEXT (NIFTY 50) ═══', '', '', '', '', '', '', '', ''])
            header_rows.append(['Start Price', round(nifty_context['start_price'], 2), '', '', '', '', '', '', ''])
            header_rows.append(['End Price', round(nifty_context['end_price'], 2), '', '', '', '', '', '', ''])
            header_rows.append(['% Change', f"{nifty_context['pct_change']:+.2f}%", '', '', '', '', '', '', ''])
            header_rows.append(['Market Regime', nifty_context['regime'], '', '', '', '', '', '', ''])
            header_rows.append(['High', round(nifty_context['high'], 2), '', '', '', '', '', '', ''])
            header_rows.append(['Low', round(nifty_context['low'], 2), '', '', '', '', '', '', ''])
            header_rows.append(['', '', '', '', '', '', '', '', ''])
        
        header_rows.append(['═══ STOCK RESULTS ═══', '', '', '', '', '', '', '', ''])
        header_rows.append(['', '', '', '', '', '', '', '', ''])
        
        df_header = pd.DataFrame(header_rows, columns=df_summary.columns)
        df_final = pd.concat([df_header, df_summary], ignore_index=True)
        ...
        df_winners = pd.DataFrame([{
            ...
            'Efficiency (%)': round(r['profit_efficiency'], 2),
        ...
        for category, stocks in pattern_data.items():
            ...
            pattern_rows.append(['Stock', 'Best Filter', 'Net Profit (₹)', 'Efficiency (%)', 'Win Rate (%)', 'Trades', 'Avg Price (₹)'])
            ...
            for stock in sorted(stocks, key=lambda x: x['net_profit'], reverse=True):
                pattern_rows.append([
                    ...
                    round(stock['profit_efficiency'], 2),
    ...
    # ═══════════════════════════════════════════════════════════════
    # ADD HYPERLINKS for Navigation - ✅ Added
    # ═══════════════════════════════════════════════════════════════
    ...
    # Add hyperlinks in TOC sheet
    toc_sheet = wb['📑 TOC']

    # Link to Summary (row 7, col A)
    toc_sheet['A7'].hyperlink = "#Summary!A1"
    toc_sheet['A7'].value = "🔗 Summary Sheet"
    toc_sheet['A7'].font = Font(color="0563C1", underline="single")

    # Link to Winners (row 8, col A)
    toc_sheet['A8'].hyperlink = "#Winners!A1"
    toc_sheet['A8'].value = "🔗 Winners Sheet"
    toc_sheet['A8'].font = Font(color="0563C1", underline="single")

    # Link to Losers (row 9, col A)
    toc_sheet['A9'].hyperlink = "#Losers!A1"
    toc_sheet['A9'].value = "🔗 Losers Sheet"
    toc_sheet['A9'].font = Font(color="0563C1", underline="single")

    # Link to Pattern Analysis (row 10, col A)
    toc_sheet['A10'].hyperlink = "#'Pattern Analysis'!A1"
    toc_sheet['A10'].value = "🔗 Pattern Analysis"
    toc_sheet['A10'].font = Font(color="0563C1", underline="single")

    # Adjust column width for TOC
    toc_sheet.column_dimensions['A'].width = 60
    toc_sheet.column_dimensions['B'].width = 50

    # Add "Back to TOC" links on each data sheet
    for sheet_name in ['Summary', 'Winners', 'Losers', 'Pattern Analysis']:
        ws = wb[sheet_name]
        # Insert a row at the top
        ws.insert_rows(1)
        ws['A1'].hyperlink = "#'📑 TOC'!A1"
        ws['A1'].value = "⬅️ Back to Table of Contents"
    ...
    print(f"   ✓ Navigation: TOC sheet with clickable links added")
    return filename
