"""
Optuna Optimization Runner — FV1 Sandbox Step 3.2
==================================================
...
Search space:
  5 boolean feature flags:
    use_position_guard   (Change 3 — 1-position-per-stock)
    use_compounding      (Change 5 — compounding position sizing)
    use_entry_cutoff     (Change 9 — entry cutoff 14:30 -> 14:45)
    use_auction_filter   (Change 10 — skip entries before 09:45)
    use_fixed_fractional (Change 11 — fixed fractional sizing)

  SL variant: A / B / C / D
    B/C: breakeven_trigger (0.5 – 5.0 ATR, step 0.5)
    D:   trailing_dist     (0.5 – 4.0 ATR, step 0.5)
"""
...
YEARS           = 4
NUM_STOCKS      = 30          # capital denominator
EXCLUDE         = {"NIFTY50", "VI"}
ATR_MULT_STOP   = 2.5         # Extreme-2
RR_TARGET       = 4.5 / 2.5  # 1.8
RISK_PER_TRADE  = 0.01
VOL_MULT        = 1.2
LOOKAHEAD       = 3
_EOD_INT        = 1500        # 15:00 as HHMM int
_CUTOFF_DEFAULT = 1430        # 14:30 as HHMM int
_CUTOFF_LATE    = 1445        # 14:45 as HHMM int
_AUCTION_INT    = 945         # 09:45 as HHMM int
...
stock_arrays = {}   # stock -> dict of numpy arrays
stock_dfs    = {}   # stock -> DataFrame (needed for signal generation)
...
# 4 combinations: (use_entry_cutoff x use_auction_filter)
# Keys: bar index (int) -> list of signal dicts — faster than datetime dict
...
def _gen_signals_by_idx(stock, use_entry_cutoff, use_auction_filter):
    """Returns {bar_index: [signal_dict]} for fast O(1) lookup in trial loop."""
    df   = stock_dfs[stock]
    ...
    # Deduplicate by entry bar index (keep first)
    seen   = set()
    sm_idx = {}
...
# signal_cache[(use_cutoff, use_auction)][stock] = {bar_idx: signal_dict}
signal_cache = {}
...
def run_trial(params):
    """
    Runs a full backtest for the given parameter set.
    Uses precomputed numpy arrays — no df.iloc[] calls.
    Returns CAGR as float.
    """
    ...
    be_trig  = params.get("breakeven_trigger")   # only for B/C
    tr_dist  = params.get("trailing_dist")        # only for D
    ...
    for stock, arrs in stock_arrays.items():
        ...
        positions = []          # each pos is a list: see indices below
        # pos layout:
        ...
        for i in range(n):
            ...
            if i in sm:
                if not (use_pg and positions):   # guard: skip if occupied
                    sig   = sm[i]
            ...
            for pos in positions:
                ...
                # EOD forced exit (same day, at/after 15:00)
                ...
                if sl_var == "B" or sl_var == "C":
                    if not pos[7]:
                        if hi >= pos[2] + be_trig * pos[6]:
                            pos[4] = pos[2]   # move stop to breakeven
                            pos[7] = True
                elif sl_var == "D":
                ...
                # Candle-direction exit priority (v1.4.5 logic)
                exit_p = None
                if is_bull:
    ...
    if final_equity <= 0:
        return -99.0   # account blown up — return floor instead of NaN
...
def objective(trial):
    params = {
        ...
        "use_fixed_fractional": False,   # excluded: shown to produce invalid sizing artifacts
    ...
    if sv in ("B", "C"):
        ...
        params["trailing_dist"]     = trial.suggest_float("trailing_dist",     1.5, 4.0, step=0.5)
...
# ─── STEP 5: WARM-UP RUN (baseline sanity check) ──────────────────────────────
...
print(f"  Baseline (E2-A, no changes): {baseline_cagr:.2f}%\n", flush=True)
...
print("\n" + "=" * 65)
print("OPTUNA RESULTS — 200 TRIALS COMPLETE")
print("=" * 65)
