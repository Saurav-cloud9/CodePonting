...
def main():
    ...
    print("║" + " " * 22 + f"{SCRIPT_NAME.upper()}" + " " * 28 + "║")
    print("╚" + "═" * 78 + "╝\n")
    ...
    if len(trades_df) > 0:
        trades_df.to_csv(f'{SCRIPT_NAME}_trades.csv', index=False)
        print(f"✓ Saved CSV: {len(trades_df):,} trades")

    if len(monthly_df) > 0:
        monthly_df.to_csv(f'{SCRIPT_NAME}_monthly.csv', index=False)
        print(f"✓ Saved CSV: {len(monthly_df)} monthly records")
    ...
    if len(consistency_df) > 0:
        consistency_df.to_csv(f'{SCRIPT_NAME}_consistency.csv', index=False)
        print(f"✓ Saved CSV: {len(consistency_df)} stocks consistency")
