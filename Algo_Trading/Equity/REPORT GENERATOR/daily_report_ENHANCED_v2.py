...
def generate_html_report(trades):
    ...
    # Calculate summary - FIXED NET P&L
    ...
    # Net P&L should be 0 if no sells yet (all positions open)
    # For open positions, show unrealized P&L separately
    net_pnl = sell_amount - buy_amount if sell_trades else 0
    
    # Get current prices for open positions (would need API call in real implementation)
    # For now, just mark as "Positions Open"
    positions_status = "Open Positions" if buy_trades and not sell_trades else "Squared Off"
    ...
    for trade in sorted(trades, key=lambda x: x.get('order_timestamp', '')):
        ...
        if ttype == "BUY":
            ...
            # LTP would need live API call - showing as TBD for now
            ltp = "Live"
            # P&L calculation (would need current price)
            pnl_display = '<span style="color: #6c757d;">Open</span>'
        else:
            target = "-"
            stoploss = "-"
            ltp = "-"
            # For sell orders, calculate actual P&L if we have the buy
            pnl_display = '<span style="color: #6c757d;">-</span>'
