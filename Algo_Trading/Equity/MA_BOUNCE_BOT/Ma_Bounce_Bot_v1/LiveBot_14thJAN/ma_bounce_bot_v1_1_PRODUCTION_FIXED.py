"""
MA BOUNCE BOT v1.1 PRODUCTION - TRUE BOUNCE LOGIC 🚀
============================================================
v1.1 (Jan 14, 2026) - TRUE BOUNCE FIX:
     ...
     ✅ Week 1 Testing: 5 shares per stock
...
ACCESS_TOKEN = "eyJ0eXAiOiJKV1QiLCJrZXlfaWQiOiJza192MS4wIiwiYWxnIjoiSFMyNTYifQ.eyJzdWIiOiJFRTY4MTkiLCJqdGkiOiI2OTY0ODA1MDA2NDFlYzdhMDY4ZTI2YzMiLCJpc011bHRpQ2xpZW50IjpmYWxzZSwiaXNQbHVzUGxhbiI6ZmFsc2UsImlhdCI6MTc2ODE5NDEyOCwiaXNzIjoidWRhcGktZ2F0ZXdheS1zZXJ2aWNlIiwiZXhwIjoxNzY4MjU1MjAwfQ.8TJ05yxbminfkbFvjA0OPYGbtNk1doUD9zVIMsBO0Io"
...
logger.info("=" * 80)
logger.info(f"MA BOUNCE BOT v1.1 PRODUCTION STARTED - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
logger.info("=" * 80)
...
def get_intraday_candles(instrument_key):
    """Get today's 1-minute intraday candles from Upstox"""
    try:
...
def get_historical_candles_from_date(instrument_key, from_date, to_date):
    """Get historical 1-minute candles from specific date"""
    try:
...
def convert_to_5min_candles(candles_1min):
    ...
    # Group candles in sets of 5
    for i in range(0, len(candles_1min), 5):
        chunk = candles_1min[i:i + 5]
...
def get_historical_candles(instrument_key, from_date, to_date):
    """
    PLATINUM CANDLE ENGINE - Fetch 1min, convert to 5min
    """
    try:
        # Fetch 1-minute intraday candles
        intraday_candles_1min = get_intraday_candles(instrument_key)

        # Convert to 5-minute
        intraday_candles_5min = convert_to_5min_candles(intraday_candles_1min)

        if len(intraday_candles_5min) >= MA_PERIOD:
            # Got enough from today
            return intraday_candles_5min

        # Not enough, get historical from previous day
        last_trading_day = get_last_trading_day()
        historical_candles_1min = get_historical_candles_from_date(
            instrument_key,
            last_trading_day,
            last_trading_day
        )

        # Convert historical to 5-minute
        historical_candles_5min = convert_to_5min_candles(historical_candles_1min)

        # Combine: intraday (newest first) + historical
        all_candles = intraday_candles_5min + historical_candles_5min
...
def check_signal(instrument_key, strategy):
    ...
    try:
        # Use Platinum Engine
        candles = get_historical_candles(instrument_key, None, None)
        ...
        for i in range(len(candles) - 4, len(candles)):
            ...
            if touch_candle['low'] <= ma20_at_touch:
                ...
                # STEP 2: BOUNCE CHECK - Check current + next candles (up to latest)
                for j in range(i, len(candles)):
                    ...
                    if bounce_candle['close'] > ma20_at_touch:
                        ...
                        return True, f"✅ BOUNCE! Price: ₹{bounce_candle['close']:.2f}, MA20: ₹{ma20_at_touch:.2f}, Distance: {distance_pct:.2f}%", distance_pct
...
def place_order(symbol, instrument_key, current_price, strategy, quantity):
            ...
            for symbol, instrument in WATCHLIST.items():
                ...
                if has_signal:
                    ...
                    if MONITOR_ONLY:
                        ...
                        order_result = place_order(symbol, instrument, live_price, strategy, 5)
                        if order_result:
                        ...
                        try:
                            ...
                            # Place order
                            order_result = place_order(symbol, instrument, live_price, strategy, quantity)
                            if order_result:
