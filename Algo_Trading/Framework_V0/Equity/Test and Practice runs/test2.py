# python
# Install into your venv (run in terminal):
# C:\Users\Saurav\CodePonting\.venv\Scripts\python.exe -m pip install openai

import os
from openai import OpenAI

# Prefer using an environment variable for the key
api_key = os.getenv("OPENAI_API_KEY", "xai-your-key-here")
...
def draw_dashboard_header():
    ...
    # Next scan countdown
    next_scan_text = "On Exit Mode" if len(dashboard_metrics["traded_symbols_today"]) >= 10 else dashboard_metrics["last_scan"]

    strategy = dashboard_metrics["current_strategy"]
    num_positions = len(dashboard_metrics["positions"])

    # Today summary
    live_wins = sum(1 for pos in dashboard_metrics['positions'].values() if pos['pnl'] > 0)
    live_losses = sum(1 for pos in dashboard_metrics['positions'].values() if pos['pnl'] < 0)
    unrealized_pnl = sum(pos['pnl'] for pos in dashboard_metrics['positions'].values())
    total_pnl = dashboard_metrics["pnl"] + unrealized_pnl

    # Colors for P&L
    pnl_color = "\033[92m" if total_pnl >= 0 else "\033[91m"

    # Fixed-width formatting for table alignment
    header_lines = [
        f"{CYAN}{'═' * 72}{RESET}",
        f"{CYAN}▀{RESET} {title:<88} {RESET}",
        for Symbol|items
        ...
        if response.status_code == 200:
            ...
            for position in positions:
                ...
                if not isinstance(position, dict):
                    print(f"⚠️Skipping non-dict position: {type(position)}")
                    continue
        ...
        try:
            ...
            for symbol, instrument in WATCHLIST.items():
                ...
                if has_signal:
                    ...
                    # v0.8 FIX: Increment signals counter
                    dashboard_metrics["signals_today"] += 1  # ← ADD THIS LINE
...
if __name__ == "__main__":
    ...
    response = client.chat.completions.create(
        model="grok-4",
        messages=[{"role": "user", "content": "Analyze X sentiment on YESBANK stock today and suggest a trade signal."}]
    )
