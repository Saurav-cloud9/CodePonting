# mega_backtest_48M_30S_v1_4_3_Optimized.py

"""
Mega Backtest Script v1.4.3 OPTIMIZED (FULL COMPLETE VERSION - SLOW & STEADY)
╔═══════════════════════════════════════════════════════════════╗
║   MEGA BACKTEST - 48 Months × 30 F&O Stocks                  ║
║   FEATURES:                                                   ║
║     1. Multiprocessing (4 Workers) - Fast & API Safe          ║
║     2. Anti-Chasing Filter (Max 0.5% from MA)                 ║
...
MAX_DIST_FROM_MA_PCT = 0.005  # 0.5% Anti-Chasing Filter
...
# ═══════════════════════════════════════════════════════════════
# DATA COLLECTION GLOBALS (For Main Thread Aggregation)
# ═══════════════════════════════════════════════════════════════
...
def check_ma_filter(row, required_mas):
    if not required_mas: return True
    for ma in required_mas:
        if pd.isna(row[ma]) or row['close'] < row[ma]: return False
    return True


# ═══════════════════════════════════════════════════════════════
# CORE LOGIC (With Optimizations)
# ═══════════════════════════════════════════════════════════════

def detect_bounce(df, filter_mas):
    """Detects bounces with ANTI-CHASING filter applied."""
    signals = []
    n = len(df)
        ...
        if low[i] <= ma20[i]:
            ...
            for j in range(i, min(i + 4, n)):
                if close_arr[j] > ma20_at_touch:
                    ...
                    # ---------------------------------------------------
                    # REFINEMENT: ANTI-CHASING FILTER
                    # ---------------------------------------------------
                    entry_price = open_arr[next_candle_idx]
                    dist_pct = (entry_price - ma20_at_touch) / ma20_at_touch

                    if dist_pct > MAX_DIST_FROM_MA_PCT:
                        # Skip this trade: Price is overextended (>0.5% from MA)
                        break
                        # ---------------------------------------------------
...
def simulate_trades(df, signals, atr_config, stock_name="", filter_name="", atr_name="", month_name=""):
    ...
    # Pre-calculate common metrics to speed up loop
    df['high_shifted'] = df['high'].shift(-1)  # Just helpers if needed, but we iterate row-by-row for accuracy

    for signal in signals:
        entry_price = signal['entry_price']
        entry_time = signal['datetime']

        # Locate entry index efficiently
        # Since we passed signals from detect_bounce, we know the time exists
        try:
        ...
        for j in range(entry_idx + 1, min(entry_idx + 80, len(df))):
            ...
            if is_bullish:
                ...
                if candle['low'] <= stop_price:
                    exit_price, exit_time, exit_reason, exit_idx = stop_price, candle['datetime'], 'SL', j
                    break
                # Check Target second
                if candle['high'] >= target_price:
                    exit_price, exit_time, exit_reason, exit_idx = target_price, candle['datetime'], 'Target', j
                    break
            else:
                # Check Target first
                if candle['high'] >= target_price:
...
def backtest_stock(df, daily_mas, stock_name, month_name=""):
    """Single Stock Backtest Function"""
    ...
    for filter_name, filter_mas in FILTERS.items():
        for atr_name, atr_config in ATR_CONFIGS.items():
            ...
            if len(signals) == 0:
                results[(filter_name, atr_name)] = {'trades': 0, 'win_rate': 0, 'net_profit': 0}
                continue
    ...
    # Re-simulate best to get specific stats
    ...
    return {
        ...
        'all_trades': all_trades_log  # Pass back ALL trades for CSV
    }
...
# ═══════════════════════════════════════════════════════════════
# MAIN EXECUTION (SLOW & STEADY APPROACH)
# ═══════════════════════════════════════════════════════════════
...
def main():
    ...
    # Generate Months (Jan 2022 - Dec 2025)
    months = []
    ...
    print(f"Workers: {MAX_WORKERS} | Anti-Chasing Filter: {MAX_DIST_FROM_MA_PCT * 100}% Active\n")

    start_time = datetime.now()
    all_top10_stocks = []

    # -------------------------------------------------------------
    # BATCH EXECUTION (MONTH-BY-MONTH)
    # -------------------------------------------------------------
    for i, (f, t, month_name) in enumerate(months, 1):
        print(f"[{i:2}/48] {month_name:<12} Processing...", end="\r")

        # 1. Create tasks JUST for this month (30 stocks)
        current_month_tasks = [(stock, key, f, t, month_name) for stock, key in STOCKS.items()]

        # 2. Submit only this batch to workers
        with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
            # .map guarantees results come back, but we just need them completed
        ...
        # 4. Save trades to global list
        for r in month_results:
            ALL_TRADES_DATA.extend(r['all_trades'])

        # 5. Sort & Print Top 10 immediately
        ...
        for rank, r in enumerate(month_results[:10], 1):
            ...
            MONTHLY_SUMMARY_DATA.append({
                ...
                'stock': r['stock'],
                'win_pct': r['win_pct'],
                'efficiency': r['capital_efficiency'],
                'net_pnl': r['net_profit'],
                'filter': r['best_filter'],
                'atr_config': r['best_atr_config']
            })
            all_top10_stocks.append(r['stock'])
        print("")  # Newline for next month
    ...
    for rank, (stock, count) in enumerate(freq.most_common(15), 1):
        consistency_pct = (count / 48) * 100
        print(f"{rank:<6} {stock:<15} {count}/48{' ' * 10} {consistency_pct:.1f}%")
    ...
    # ═══════════════════════════════════════════════════════════════
    print("\n" + "=" * 70)
    print(" SAVING DATA TO CSV & DATABASE")
    print("=" * 70)
    ...
    # Save CSVs
    if len(trades_df) > 0:
        trades_df.to_csv('mega_backtest_48M_30S_v1_4_3_optimized_trades.csv', index=False)
        print(f"✓ Saved: mega_backtest_48M_30S_v1_4_3_optimized_trades.csv ({len(trades_df):,} trades)")

    if len(monthly_df) > 0:
        monthly_df.to_csv('mega_backtest_48M_30S_v1_4_3_optimized_monthly.csv', index=False)
        print(f"✓ Saved: mega_backtest_48M_30S_v1_4_3_optimized_monthly.csv ({len(monthly_df)} records)")

    # Create stock consistency summary
    stock_consistency = []
    all_top10_for_consistency = [r['stock'] for r in MONTHLY_SUMMARY_DATA if r['rank'] <= 10]
    stock_freq = Counter(all_top10_for_consistency)

    for stock, count in stock_freq.most_common():
        consistency_pct = (count / 48) * 100
        stock_consistency.append({
            'stock': stock,
            'top10_appearances': count,
            'consistency_pct': consistency_pct
        })

    consistency_df = pd.DataFrame(stock_consistency)
    if len(consistency_df) > 0:
        consistency_df.to_csv('mega_backtest_48M_30S_v1_4_3_optimized_consistency.csv', index=False)
        print(f"✓ Saved: mega_backtest_48M_30S_v1_4_3_optimized_consistency.csv ({len(consistency_df)} stocks)")

    # Save to SQLite Database
    db_path = 'mega_backtest_48M_30S_v1_4_3_optimized.db'
    conn = sqlite3.connect(db_path)

    if len(trades_df) > 0:
        trades_df.to_sql('trades', conn, if_exists='replace', index=False)
        print(f"✓ Database: trades table ({len(trades_df):,} rows)")

    if len(monthly_df) > 0:
        monthly_df.to_sql('monthly_summary', conn, if_exists='replace', index=False)
        print(f"✓ Database: monthly_summary table ({len(monthly_df)} rows)")

    if len(consistency_df) > 0:
        consistency_df.to_sql('stock_consistency', conn, if_exists='replace', index=False)
        print(f"✓ Database: stock_consistency table ({len(consistency_df)} rows)")
    ...
    print("=" * 70)
    print("\nFiles created:")
    print("  1. mega_backtest_48M_30S_v1_4_3_optimized_trades.csv")
    print("  2. mega_backtest_48M_30S_v1_4_3_optimized_monthly.csv")
    print("  3. mega_backtest_48M_30S_v1_4_3_optimized_consistency.csv")
    print("  4. mega_backtest_48M_30S_v1_4_3_optimized.db")
    print("\nReady for analysis!")
