...
"""
Mega Backtest Script v1.4.3 SAFE OPTIMIZED + DATA COLLECTION
╔═══════════════════════════════════════════════════════════════╗
║   MEGA BACKTEST - 48 Months × 30 F&O Stocks (CONSOLE ONLY)    ║
║   OUTPUTS: Console + CSV + SQLite DB for Analysis              ║
╚═══════════════════════════════════════════════════════════════╝
"""
...
def simulate_trades(df, signals, atr_config, stock_name="", filter_name="", atr_name="", month_name=""):
    ...
    for signal in signals:
        ...
        exit_time = None
        exit_reason = None
        exit_idx = None  # Track exit index for candles_held calculation
        ...
        candles_held = exit_idx - entry_idx if exit_idx is not None else 0
...
def backtest_stock(df, daily_mas, stock_name, month_name=""):
    ...
    best_trades = simulate_trades(df, best_signals, ATR_CONFIGS[best_atr_config], stock_name, best_filter, best_atr_config, month_name)
...
def main():
    ...
    for month_idx, (from_date, to_date, month_name) in enumerate(months, 1):
        ...
        for rank, r in enumerate(month_results[:10], 1):
            ...
            # Collect monthly summary data for this month's top 10
            MONTHLY_SUMMARY_DATA.append({
                'month': month_name,
                'rank': rank,
                'stock': r['stock'],
                'trades': r['trades'],
                'targets': r['target_hits'],
                'sl_hits': r['sl_hits'],
                'eod_exits': r['eod_exits'],
                'win_pct': r['win_pct'],
                'efficiency': r['capital_efficiency'],
                'protrades_pct': r['protrades_pct'],
    ...
    # Save CSVs
    if len(trades_df) > 0:
        trades_df.to_csv('backtest_trades_all_v1_4_3.csv', index=False)
        print(f"✓ Saved: backtest_trades_all_v1_4_3.csv ({len(trades_df):,} trades)")
    
    if len(monthly_df) > 0:
        monthly_df.to_csv('backtest_monthly_top10_v1_4_3.csv', index=False)
        print(f"✓ Saved: backtest_monthly_top10_v1_4_3.csv ({len(monthly_df)} records)")
    
    # Create stock consistency summary
    stock_consistency = []
    all_top10_stocks = [r['stock'] for r in MONTHLY_SUMMARY_DATA if r['rank'] <= 10]
    stock_freq = Counter(all_top10_stocks)
    ...
    if len(consistency_df) > 0:
        consistency_df.to_csv('backtest_stock_consistency_v1_4_3.csv', index=False)
        print(f"✓ Saved: backtest_stock_consistency_v1_4_3.csv ({len(consistency_df)} stocks)")
    
    # Save to SQLite Database
    db_path = 'backtest_results_v1_4_3.db'
    ...
    print("="*70)
    print("\nFiles created:")
    print("  1. backtest_trades_all_v1_4_3.csv")
    print("  2. backtest_monthly_top10_v1_4_3.csv")
    print("  3. backtest_stock_consistency_v1_4_3.csv")
    print("  4. backtest_results_v1_4_3.db")
