# mega_backtest_48M_30S_v1_4_6.py

"""
Mega Backtest Script v1.4.6 — 30 F&O Stocks (PARQUET + ALL FIXES)
╔═══════════════════════════════════════════════════════════════╗
║   MEGA BACKTEST - 48 Months × 30 F&O Stocks                  ║
║   Based on: mega_backtest_48M_30S_v1_4_5_vedanta.py          ║
║   FIXES vs v1.4.5:                                            ║
║     1. Rolling indicators on FULL continuous dataset          ║
║        (no NaN warm-up gap at month boundaries)               ║
║     2. detect_bounce starts from index 0 (not 20)            ║
║     3. Global dedup of signals across months (per stock)      ║
║     4. EOD exit: open of first bar at/after 15:00            ║
║     5. No entries at/after 14:30                              ║
║     6. Cross-month signal guard                               ║
║   Output: v146_30stocks_results.csv                           ║
╚═══════════════════════════════════════════════════════════════╝
"""
...
# ═══════════════════════════════════════════════════════════════
# DATA LOADING  (indicators on full continuous dataset — the fix)
# ═══════════════════════════════════════════════════════════════

def load_intraday_data(parquet_path):
    ...
    # Rolling indicators on FULL continuous dataset (warm-up fix)
...
def get_month_slice(df_full, from_date, to_date):
    ...
    return df_full[mask].copy().reset_index(drop=True)


# ═══════════════════════════════════════════════════════════════
# CORE LOGIC  (fixed: starts from 0, global dedup, 14:30 cutoff,
#              cross-month guard)
# ═══════════════════════════════════════════════════════════════
...
def detect_bounce(df, filter_mas, seen_times=None):
    ...
    # FIX: start from 0 — indicators already warm on full dataset
    for i in range(0, n):
        ...
        if low[i] <= ma20[i]:
            ...
            for j in range(i, min(i + 4, n)):
                if close_arr[j] > ma20_at_touch:
                    ...
                    # FIX: cross-month guard
                    if (pd.Timestamp(datetime_arr[i]).year,  pd.Timestamp(datetime_arr[i]).month) != \
                       (pd.Timestamp(datetime_arr[next_candle_idx]).year,
                        pd.Timestamp(datetime_arr[next_candle_idx]).month):
                        break

                    # FIX: no entries at/after 14:30
                    if pd.Timestamp(datetime_arr[next_candle_idx]).time() >= \
    ...
    # FIX: global dedup — seen_times persists across month calls
...
def simulate_trades(df, signals, atr_config, stock_name="", filter_name="", atr_name="", month_name=""):
    ...
    for signal in signals:
        ...
        for j in range(entry_idx + 1, len(df)):
            ...
            # FIX: EOD forced close — first bar at/after 15:00 on the entry date
            if candle['datetime'].date() == entry_date and \
               candle['datetime'].time() >= pd.Timestamp('15:00').time():
                exit_price, exit_time, exit_reason, exit_idx = \
                    candle['open'], candle['datetime'], 'EOD', j
                break
        ...
        if exit_price is None:
            continue  # end-of-dataset with no exit — skip
...
def backtest_stock_month(df, daily_mas, stock_name, month_name, seen_times_dict):
    ...
    for filter_name, filter_mas in FILTERS.items():
        for atr_name, atr_config in ATR_CONFIGS.items():
            st      = seen_times_dict.get((filter_name, atr_name))
...
def main():
    print("\n" + "=" * 70)
    print("  48-MONTH BACKTEST — ALL STOCKS  [v1.4.6  FIXED]")
    print("=" * 70 + "\n")
    ...
    for stock_path in stock_files:
        ...
        try:
            df_full    = load_intraday_data(str(stock_path))
            daily_mas  = compute_daily_mas(df_full)

            # Per-stock global dedup dict (fresh per stock, shared across its 48 months)
            seen_times_dict = {
                (fn, an): set() for fn in FILTERS for an in ATR_CONFIGS
            }

            stock_trades = []
            for f, t, month_name in months:
                month_df = get_month_slice(df_full, f, t)
                trades   = backtest_stock_month(month_df, daily_mas, stock,
                                                month_name, seen_times_dict)
            ...
            for atr_name in ATR_CONFIGS:
                ...
                pnl_per_share = total_pnl / trade_count  # pnl is already per-share (no qty)
    ...
    out_path = os.path.join(SCRIPT_DIR, 'v146_30stocks_results.csv')
    ...
    if master_trades:
        ...
        trades_out = os.path.join(SCRIPT_DIR, 'v146_all_trades.csv')
