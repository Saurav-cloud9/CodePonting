...
def calculate_gss(price, ma20, ema200, ma20_5d_ago, adx, prev_adx, rsi, prev_rsi, plus_di, minus_di, prev_plus_di):
    """
    GSS v3.2: Alpha-Generating System with Fresh Momentum Filtering
    Returns: score (0-100)
    
    Changes from v3.1:
    - Fresh momentum filter: ADX points only if +DI > -DI AND +DI > prev_+DI (accelerating bullish momentum)
    - Prevents scoring "exhausted" trends that are reversing
    - Maintains directional filter and RSI exhaustion penalty
    """
    ...
    # Factor 3A: ADX Strength (15 pts) - FRESH BULLISH MOMENTUM REQUIRED
    # Critical: Only award if +DI > -DI AND +DI is RISING (not exhausted)
    if adx > 20 and plus_di > minus_di and plus_di > prev_plus_di:
        score += 15
    
    # Factor 3B: ADX Acceleration (15 pts) - FRESH BULLISH MOMENTUM REQUIRED
...
def map_score_to_regime(score, volume, volume_ma20, adx_slope, rsi, plus_di, minus_di, prev_plus_di, prev_minus_di):
    """
    GSS v3.2: Triple-Lock Regime Mapping with Symmetric BULL/BEAR Logic
    
    BULL Requirements (4-way lock):
    1. Score >= 75
    2. +DI > -DI (bullish direction)
    3. +DI > prev_+DI (accelerating bullish momentum)
    4. Volume confirmed
    
    BEAR Requirements (4-way lock - SYMMETRIC):
    1. Score <= 25
    2. -DI > +DI (bearish direction)
    3. -DI > prev_-DI (accelerating bearish momentum)
    4. Volume confirmed
    
    SIDEWAYS: Everything else (quality filter - avoid noise)
    """
    ...
    # Re-tightened volume threshold (v3.1 was too loose at 1.0x-1.05x)
    if adx_slope > 0 and rsi > 50:  # Confirmed momentum
        volume_threshold = 1.08  # Relaxed for momentum (was 1.0x in v3.1)
    else:
        volume_threshold = 1.15  # Standard requirement (was 1.05x in v3.1)
    
    volume_confirmed = volume > (volume_ma20 * volume_threshold)
    
    # Triple-Lock Regime Mapping
    ...
    # BEAR: Fresh bearish momentum + volume (SYMMETRIC to BULL)
    elif (score <= 25 and 
    ...
    # SIDEWAYS: Everything else (quality filter)
    else:
        return "SIDEWAYS"
...
def validate_gss():
    ...
    for i in range(1, len(nifty)):
        ...
        # Get previous values (from day before N-1) for momentum detection
        if i >= 2:
            prev_adx = nifty.iloc[i - 2]['ADX']
            prev_rsi = nifty.iloc[i - 2]['RSI']
            prev_plus_di = nifty.iloc[i - 2]['Plus_DI']   # NEW for fresh momentum
            prev_minus_di = nifty.iloc[i - 2]['Minus_DI']  # NEW for fresh momentum
        else:
        ...
        # Calculate GSS score using N-1 data with fresh momentum filtering
        score = calculate_gss(
            ...
            prev_plus_di=prev_plus_di  # NEW
        )

        # Apply Triple-Lock regime mapping with symmetric BULL/BEAR requirements
        prediction = map_score_to_regime(
            ...
            minus_di=prev_row['Minus_DI'],
            prev_plus_di=prev_plus_di,  # NEW
            prev_minus_di=prev_minus_di  # NEW
        )
    ...
    # Step 8: Decision
    print("═══════════════════════════════════════════════════════════════════════════════")
    print("VALIDATION METHOD: GSS v3.2 Triple-Lock System + ATR-based Future-Truth")
    print("Regime labels: BULL/BEAR if 5-day move > 1.5×ATR, else SIDEWAYS")
    print("")
    print("GSS v3.2 Changes (Triple-Lock Quality Filter):")
    print("  • Fresh momentum filter: +DI > prev_+DI (accelerating, not exhausted)")
    print("  • BULL requirements: Score ≥75 + +DI>-DI + +DI rising + Volume 1.08x-1.15x")
    print("  • BEAR requirements: Score ≤25 + -DI>+DI + -DI rising + Volume 1.08x-1.15x")
    print("  • Symmetric BULL/BEAR logic prevents directional bias")
    print("  • Volume re-tightened from v3.1 (1.08x confirmed, 1.15x standard)")
    print("  • SIDEWAYS: Everything else (quality filter - precision over recall)")
    print("─────────────────────────────────────────────────────────────────────────────")
    if accuracy >= 70:
