...
def calculate_gss(price, ma20, ema200, ma20_5d_ago, adx, prev_adx, rsi, prev_rsi):
    """
    GSS v3.0: Alpha-Generating Predictive System
    Replaces lagging confirmations with leading momentum indicators
    Returns: score (0-100)
    
    Changes from v2.0:
    - Added RSI momentum (20 pts) - leading indicator
    - Split ADX into strength + acceleration (15+15 pts)
    - Relaxed thresholds: MA slope 0.05%, ADX 20, proximity 3%
    - Rebalanced weights for earlier signal generation
    """
    ...
    # Factor 1: Long-term Anchor (15 pts) - Reduced weight
    if price > ema200:
        score += 15

    # Factor 2: MA20 Slope (20 pts) - Relaxed threshold
    ma_slope_pct = ((ma20 - ma20_5d_ago) / ma20_5d_ago) * 100
    if ma_slope_pct > 0.05:  # Relaxed from 0.1%
        score += 20

    # Factor 3A: ADX Strength (15 pts) - Lowered threshold
    if adx > 20:  # Lowered from 25
        score += 15
    
    # Factor 3B: ADX Acceleration (15 pts) - LEADING INDICATOR
    if adx > prev_adx and adx > 15:
        score += 15

    # Factor 4A: RSI Above Equilibrium (10 pts) - NEW LEADING INDICATOR
    if rsi > 50:
        score += 10
    
    # Factor 4B: RSI Rising (10 pts) - NEW LEADING INDICATOR
    if rsi > prev_rsi:
        score += 10

    # Factor 5: Price Proximity (15 pts) - Relaxed range
...
def map_score_to_regime(score, volume, volume_ma20, adx_slope):
    """
    Convert GSS score to regime label with dynamic volume confirmation
    Volume threshold adjusts based on ADX momentum strength
    """
    ...
    # Dynamic volume threshold based on momentum
    if adx_slope > 5:  # Strong directional acceleration
        volume_threshold = 1.1  # Relaxed requirement
    else:
        volume_threshold = 1.2  # Standard requirement
...
def validate_gss():
    ...
    for i in range(1, len(nifty)):
        ...
        # Apply volume-aware regime mapping with dynamic volume filter
    ...
    if len(mismatches) > 0:
        ...
        print("─────────────────────────────────────────────────────────────────────────────")
        display_cols = ['Close', 'MA20', 'EMA200', 'ADX', 'RSI', 'GSS_Score', 'Actual_Regime', 'GSS_Prediction']
    ...
    # Step 8: Decision
    print("═══════════════════════════════════════════════════════════════════════════════")
    print("VALIDATION METHOD: GSS v3.0 with Leading Indicators + ATR-based Future-Truth")
    print("Regime labels: BULL/BEAR if 5-day move > 1.5×ATR, else SIDEWAYS")
    print("GSS v3.0 Changes:")
    print("  • RSI momentum (20 pts) - leading indicator for early trend detection")
    print("  • ADX split: strength (15 pts) + acceleration (15 pts)")
    print("  • Relaxed thresholds: MA slope 0.05%, ADX 20, proximity 3%")
    print("  • Dynamic volume filter: 1.1x for high momentum, 1.2x standard")
