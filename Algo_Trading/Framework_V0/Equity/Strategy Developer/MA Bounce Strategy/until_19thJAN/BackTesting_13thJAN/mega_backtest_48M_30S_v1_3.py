...
def backtest_stock(df, daily_mas, stock_name):
    ...
    losses = sum(1 for t in best_trades if t['pnl'] <= 0)
...
def main():
    ...
    for month_idx, (from_date, to_date, month_name) in enumerate(months, 1):
        ...
        print("\n  🏆 TOP 10:")
        print(f"  {'Rank':<6} {'Stock':<12} {'Trades':<7} {'Wins':<5} {'Loss':<5} {'Win%':<6} {'Net₹':<10} {'Capital₹':<12} {'Eff%':<6} {'GE':<20} {'Filter':<12} {'Target':<8}")
        print("  " + "-" * 120)
        for rank, r in enumerate(month_results[:10], 1):
            ...
            print(f"  {rank:<6} {r['stock']:<12} {r['trades']:<7} {r['wins']:<5} {r['losses']:<5} {win_pct:<6.0f} {r['net_profit']:<10.0f} {r['total_capital']:<12.0f} {r['capital_efficiency']:<6.1f} {ge_str:<20} {r['best_filter']:<12} {r['best_target']:.1f}%")
