...
def draw_dashboard_header():
    ...
    # v0.8: Show "On Exit Mode" when 10/10 trades
    trades_today = len(dashboard_metrics.get("traded_symbols_today", set()))
    MAX_TRADES_PER_DAY = 10

    if trades_today >= MAX_TRADES_PER_DAY:
        scan_text = "Next scan: On Exit Mode"
    else:
        scan_text = f"Next scan: {next_scan:5s}"

    strategy = dashboard_metrics["current_strategy"]
    num_positions = len(dashboard_metrics["positions"])

    # v0.8: Calculate live wins/losses/P&L from open positions
    ...
    total_pnl = dashboard_metrics["pnl"] + unrealized_pnl  # Realized + Unrealized
    ...
    line1 = f"Ma_Bounce_Bot_v0_8 🎮"
    line2 = f"{time_str} | {strategy:17s} | {scan_text}"
    line3 = f"TODAY: Signals: {dashboard_metrics['signals_today']:2d} | Trades: {dashboard_metrics['trades_today']:2d} | Win: {live_wins:2d} | Loss: {live_losses:2d} | P&L: {pnl_color}₹{total_pnl:+.2f}{WHITE}"
