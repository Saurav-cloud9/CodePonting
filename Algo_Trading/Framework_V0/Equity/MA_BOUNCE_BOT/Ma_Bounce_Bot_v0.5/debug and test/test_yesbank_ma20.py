...
def main():
    ...
    # Take last 100 candles (as the bot would do)
    print("\n" + "=" * 80)
    print("STEP 1: Take last 100 candles")
    print("=" * 80)
    
    candles_100 = all_candles[-100:] if len(all_candles) >= 100 else all_candles
    print(f"\nExtracted {len(candles_100)} candles")
    ...
    print("\nFirst 3 five-minute candles:")
    for i in range(min(3, len(candles_5min))):
        ts = candles_5min[i]['timestamp']
        dt = datetime.fromisoformat(ts.replace('Z', '+00:00'))
        print(f"  [{i}] {dt.strftime('%b %d %H:%M')} - O:{candles_5min[i]['open']:.2f} H:{candles_5min[i]['high']:.2f} L:{candles_5min[i]['low']:.2f} C:{candles_5min[i]['close']:.2f}")
    
    print("\nLast 3 five-minute candles:")
    for i in range(max(0, len(candles_5min) - 3), len(candles_5min)):
        ts = candles_5min[i]['timestamp']
        dt = datetime.fromisoformat(ts.replace('Z', '+00:00'))
        print(f"  [{i}] {dt.strftime('%b %d %H:%M')} - O:{candles_5min[i]['open']:.2f} H:{candles_5min[i]['high']:.2f} L:{candles_5min[i]['low']:.2f} C:{candles_5min[i]['close']:.2f}")
