...
def draw_dashboard_header():
    ...
    reset = "\033[0m"
    cyan = "\033[36m"  # Softer cyan for borders
    
    header = f"""{cyan}╔══════════════════════════════════════════════════════════════════════════╗
║ MA Bounce Bot v0.5 - LIVE TRADING                                        ║
║ {time_str} | {strategy:17s} | Next scan: {next_scan:5s}              ║
╠══════════════════════════════════════════════════════════════════════════╣{reset}
║ TODAY: Signals: {dashboard_metrics['signals_today']:2d} | Trades: {dashboard_metrics['trades_today']:2d} | Win: {dashboard_metrics['wins']:2d} | Loss: {dashboard_metrics['losses']:2d} | P&L: {pnl_color}₹{total_pnl:+.2f}{reset}   ║
║ POSITIONS: {num_positions} active{' ' * 55}║
"""
    ...
    if dashboard_metrics["positions"]:
        for symbol, pos in dashboard_metrics["positions"].items():
            ...
            if pnl_pct >= pos['target_pct']:
                status = f"{cyan}[T]{reset}"
            elif pnl_pct <= -pos['sl_pct']:
                status = "\033[91m[SL]\033[0m"
            else:
                status = "   "
            
            line = f"║  └─ {symbol:10s} {pos['qty']:2d}@₹{pos['entry']:.2f} → ₹{pos['current']:.2f} ({pnl_sign}{pnl_pct:+.2f}%) {status}"
            header += line + " " * (74 - len(line)) + "║\n"
