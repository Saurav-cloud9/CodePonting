...
def get_historical_candles_fixed(instrument, num_minutes=100, up_to_time=None):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if up_to_time:
                # Upstox timestamps are in ISO format with UTC timezone
                # Example: "2024-12-24T10:50:00+05:30"

                # Parse the entry time as IST (UTC+5:30)
                # Create a datetime object for today at the entry time in IST
                today = datetime.now()
                ...
                entry_dt = datetime(today.year, today.month, today.day,
                                   entry_hour, entry_minute, entry_second,
                                   tzinfo=IST)

                print(f"  [DEBUG] Filtering candles up to: {entry_dt.strftime('%I:%M:%S %p IST')}")

                # Filter candles that are at or before entry time
                filtered_candles = []
                for candle in candles:
                    # Parse candle timestamp (it comes in ISO format with timezone)
