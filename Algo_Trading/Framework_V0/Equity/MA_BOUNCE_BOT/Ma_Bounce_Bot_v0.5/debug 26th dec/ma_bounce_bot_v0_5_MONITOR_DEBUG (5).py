...
def get_historical_candles(instrument, num_minutes=100):
    ...
    try:
        ...
        print(f"DEBUG: URL - {to_date} (toDate in URL pos 1) / {from_date} (fromDate in URL pos 2)")
        ...
        print(f"DEBUG: Historical API Status: {response.status_code}")
        
        if response.status_code != 200:
            print(f"DEBUG: API Error: {response.text[:500]}")
            return []
        ...
        if response.status_code == 200:
            ...
            if 'data' not in data:
                print(f"DEBUG: No 'data' in response! Full: {data}")
                return []
            
            print(f"DEBUG: data keys: {list(data['data'].keys()) if isinstance(data['data'], dict) else 'not a dict'}")
            
            if not data['data'] or 'candles' not in data['data']:
                print(f"DEBUG: No candles! data content: {data['data']}")
                return []
                
            candles_raw = data['data']['candles']
            print(f"DEBUG: Got {len(candles_raw)} candles")
...
def get_ma20(instrument):
    ...
    try:
        ...
        if len(candles_1min) < 100:
            print(f"DEBUG: Only got {len(candles_1min)} candles, need 100")
            return None

        # DEBUG: Save the 100 one-minute candles to CSV
        import csv
        csv_filename = "bot_candles_1min_raw.csv"
        with open(csv_filename, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Index', 'Timestamp', 'Open', 'High', 'Low', 'Close', 'Volume', 'Order'])
            for i, c in enumerate(candles_1min):
                writer.writerow([i, c['timestamp'], c['open'], c['high'], c['low'], c['close'], c['volume'], 'NEWEST_FIRST'])
        print(f"📝 Saved 100 raw 1-min candles to {csv_filename} (API order - newest first)")

        # ✨ FIX: REVERSE to chronological order (oldest first, newest last)
        candles_1min = list(reversed(candles_1min))

        # DEBUG: Save reversed candles
        csv_filename_reversed = "bot_candles_1min_reversed.csv"
        with open(csv_filename_reversed, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Index', 'Timestamp', 'Open', 'High', 'Low', 'Close', 'Volume', 'Order'])
            for i, c in enumerate(candles_1min):
                writer.writerow([i, c['timestamp'], c['open'], c['high'], c['low'], c['close'], c['volume'], 'OLDEST_FIRST'])
        print(f"📝 Saved 100 reversed 1-min candles to {csv_filename_reversed} (chronological)")

        candles_5min = convert_to_5min_candles(candles_1min)
        if len(candles_5min) < 20:
            print(f"DEBUG: Only got {len(candles_5min)} 5-min candles, need 20")
            return None

        # DEBUG: Save the 5-minute candles
        csv_filename_5min = "bot_candles_5min.csv"
        with open(csv_filename_5min, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Index', 'Timestamp', 'Open', 'High', 'Low', 'Close', 'Volume', 'Used_for_MA20'])
            for i, c in enumerate(candles_5min):
                used = 'YES' if i >= len(candles_5min) - 20 else 'NO'
                writer.writerow([i, c['timestamp'], c['open'], c['high'], c['low'], c['close'], c['volume'], used])
        print(f"📝 Saved {len(candles_5min)} 5-min candles to {csv_filename_5min}")

        last_20_closes = [c['close'] for c in candles_5min[-20:]]
        ma20 = round(sum(last_20_closes) / 20, 2)
        
        print(f"\n   MA20 Calculation:")
        print(f"   Last 20 five-min candles used:")
        for i, c in enumerate(candles_5min[-20:], 1):
            print(f"   {i:2d}. {c['timestamp']} Close: ₹{c['close']:.2f}")
        print(f"   Sum: ₹{sum(last_20_closes):.2f}")
