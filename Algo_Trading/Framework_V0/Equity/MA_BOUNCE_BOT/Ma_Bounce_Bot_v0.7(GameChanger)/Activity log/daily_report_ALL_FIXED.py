...
def generate_html_report(trades, position_pnl):
    ...
    for trade in sorted(trades, key=lambda x: x.get('order_timestamp', '')):
        ...
        # Try multiple fields for price (SELL orders might use different fields)
        price = (
            float(trade.get('average_price', 0)) or
            float(trade.get('price', 0)) or
            float(trade.get('last_price', 0)) or
            0
        )
        ...
        if ttype == "BUY":
            ...
            # Check if target reached (compare actual LTP to target)
            if symbol_clean in position_pnl:
                ltp_price = round((actual_pnl_total / qty) + price, 2) if qty > 0 else price
                ...
                # No live position data - can't determine
            ...
            # SELL ORDER - Need to find matching BUY to calculate P&L
            
            # Find matching BUY order for this symbol to calculate P&L
            buy_order = None
            for t in trades:
                if (t.get('trading_symbol', '').replace('-EQ', '') == symbol_clean and 
                    t.get('transaction_type') == 'BUY'):
                    buy_order = t
                    break
            
            if buy_order and price > 0:
                buy_price = (
                    float(buy_order.get('average_price', 0)) or
                    float(buy_order.get('price', 0)) or
                    0
                )
                
                if buy_price > 0:
                    # Calculate actual P&L
                    pnl_per_share = round(price - buy_price, 2)
                    pnl_total = round(pnl_per_share * qty, 2)
                    
                    # Calculate what target and SL would have been
                    target_price = round(buy_price * 1.02, 2)  # 2% target
                    sl_price = round(buy_price * 0.99, 2)      # 1% SL
                    
                    # Check if target was reached (sell price >= target)
                    target_reached = price >= target_price
                    sl_triggered = price <= sl_price
                    
                    # Target column - show target price
                    target = f'₹{target_price:.2f}'
                    
                    # Target Profit column - show YES/NO based on if target reached
                    if target_reached:
                        target_profit_display = '<span style="color: #27ae60; font-weight: 700;">✓ YES</span>'
                    else:
                        target_profit_display = '<span style="color: #6c757d;">✗ No</span>'
                    
                    # Target Reached column - same as Target Profit
                    target_reached_display = target_profit_display
                    
                    # Stop Loss column - show YES if SL triggered, NO otherwise
                    stoploss = f'₹{sl_price:.2f}'
                    if sl_triggered:
                        stoploss = '<span style="color: #e74c3c; font-weight: 700;">✓ YES (SL Hit)</span>'
                    else:
                        stoploss = '<span style="color: #6c757d;">✗ No</span>'
                    
                    # LTP shows actual sell price
                    ltp = f'₹{price:.2f}'
                    ...
                    # Total P&L display
                    if pnl_total > 0:
                        pnl_total_display = f'<span style="color: #27ae60; font-weight: 600;">+₹{pnl_total:.2f}</span>'
                    elif pnl_total < 0:
                        pnl_total_display = f'<span style="color: #e74c3c; font-weight: 600;">₹{pnl_total:.2f}</span>'
                    else:
                        pnl_total_display = '<span style="color: #6c757d;">₹0.00</span>'
                else:
                    # Couldn't get buy price
                    target = "-"
                    stoploss = "-"
                    ltp = f'₹{price:.2f}' if price > 0 else "-"
                    target_profit_display = "-"
                    target_reached_display = "-"
                    pnl_per_share_display = '<span style="color: #6c757d;">-</span>'
                    pnl_total_display = '<span style="color: #6c757d;">Squared</span>'
            else:
                # No matching buy found or no sell price
                target = "-"
                stoploss = "-"
                ltp = f'₹{price:.2f}' if price > 0 else "-"
                target_profit_display = "-"
