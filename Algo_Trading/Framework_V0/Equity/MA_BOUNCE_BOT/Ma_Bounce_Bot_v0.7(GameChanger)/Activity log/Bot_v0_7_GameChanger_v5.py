...
v5 - CRITICAL FIXES (Dec 30, 2025 12:45 PM):
     ✅ Broker position sync at startup (prevents SAIL duplicate order bug)
        ...
        for pos in positions_data:
            ...
            if qty > 0 and symbol in WATCHLIST:  # Long position exists in our watchlist
                ...
                dashboard_metrics["positions"][symbol] = {
                    ...
                    "current": avg_price,
                    "pnl": 0.0,
                    "target_pct": 2.0,  # Default to MORNING_BOUNCE params
                    "sl_pct": 1.0,
                    "strategy": "EXISTING_BROKER",
                    "from_broker": True  # CRITICAL FLAG - prevents new orders
                }
                existing_count += 1
...
def run_bot():
    ...
    if MONITOR_ONLY:
        ...
        print("✨ v0.7 v5: Platinum + Risk Mgmt + Audio + All Critical Fixes")
    print("Strategies:")
    ...
    while True:
        ...
        try:
            ...
            # Check if we've hit max positions (v0.7 v5 FIX - use function)
