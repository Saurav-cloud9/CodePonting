...
v5 - CRITICAL FIXES (Dec 30, 2025 12:45 PM):
     ✅ Upstox position sync at startup (prevents SAIL duplicate order bug)
     ✅ Position existence check before placing orders
     ✅ Fixed position counter (get_active_position_count)
     ✅ Fixed TARGET exit logging to CSV
     ✅ Fixed MAX_POSITIONS logic (counts only active positions)
v6 - ML OPTIMIZER + TERMINOLOGY (Dec 30, 2025 01:30 PM):
     ✅ MAX_POSITIONS: 5 → 10 (faster ML data collection - 70 days to 700 trades!)
     ✅ Terminology: "broker" → "upstox" everywhere (clarity)
     ✅ Fix avg_price = 0.00 (use buy_average from Upstox API)
     ✅ Clear from_upstox flag after exit (allows re-trading closed positions)
v7 - PRODUCTION READY (Dec 30, 2025 03:15 PM):
     ✅ Fixed sync error (handle non-dict items from Upstox API)
     ✅ Fixed Unicode logger (₹ → Rs in all logger.info calls)
     ✅ Added CSV trade logging (trades_log.csv)
     ✅ LATE_SCALP disabled (insufficient volume data)
     ✅ Debug output for position loading
     ✅ Track traded_symbols_today properly
============================================================
v0.7 VERSION HISTORY:
v1 - Initial Game Changer release
v2 - Added audio beep alerts on signal detection
    ...
    try:
        ...
        if strategy == "MORNING_BOUNCE":
            ...
            if len(candles) < 2:
                ...
                return False, "LATE_SCALP: Using price-only mode (no volume data available)"
...
def place_exit_order(symbol, instrument, quantity, current_price, exit_type):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # v0.7 v5 FIX: LOG TO CSV (was missing for TARGET exits!)
            ...
            # Log to file (if you have a log_trade function)
            try:
                # v6 FIX: Simple CSV logging
                import csv
                import os
                ...
                with open(log_file, 'a', newline='', encoding='utf-8') as f:
                    ...
                    if not file_exists:
                        writer.writerow(
                            ['Date', 'Time', 'Symbol', 'Entry', 'Exit', 'Qty', 'P&L', 'Result', 'Exit_Type'])
                ...
                print(f"✅ Trade logged: {symbol} {result_status} ₹{profit:+.2f}")
        ...
        try:
            ...
            if dashboard_metrics["positions"]:
                ...
                if trades_today >= MAX_TRADES_PER_DAY:
                    print(f"\n⚠️  MAX TRADES REACHED ({trades_today}/{MAX_TRADES_PER_DAY})")
                    print("   No more new trades today, monitoring exits only...")
                    time.sleep(60)
                    continue
