...
MAX_TR_GAP = 3
EOD_HOUR   = 15
LAST_TOUCH_TIME   = _time(14, 45)  # matches live bot (ma_rejection_v1_core.py) — touch bar
                                    # must be <= 14:45, stricter than plain hour<EOD_HOUR
...
def run_stock(arr):
    ...
    while i < n:
        if np.isnan(ma20[i]) or np.isnan(atr14[i]):
            i += 1; continue
        if high[i] >= ma20[i] and time_[i] <= LAST_TOUCH_TIME:
            ...
            for k in range(ei, n):
                if date[k] != touch_date:
                    exit_px = close[k - 1]; pnl = entry - exit_px; break
                if hour[k] >= EOD_HOUR:
...
def calc_metrics(trades):
    ...
    tdf['zpnl'] = tdf.apply(lambda r: r['pnl'] - zerodha_short(r['entry'], r['exit_px']), axis=1)
...
all_df['zpnl'] = all_df.apply(lambda r: r['pnl'] - zerodha_short(r['entry'], r['exit_px']), axis=1)
