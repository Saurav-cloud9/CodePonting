"""
Generate tb3 signal CSVs for 25 stocks × years 2023/2024/2025.
5 stocks already exist (pnb, nationalum, powergrid, hdfcbank, itc).
Uses same pipeline as export_h5_signals_batch.py.
"""
import pandas as pd
import numpy as np
import os
...
STOCKS = [
    ...
    'HINDALCO',   'ICICIBANK',  'INDUSINDBK', 'INFY',       'JSWSTEEL',
    'NTPC',       'ONGC',       'RELIANCE',   'SBIN',       'SUNPHARMA',
...
def export_signals(stock, year, max_tb_gap):
    ...
    if not os.path.exists(csv_path):
        print(f"  SKIP {stock} — CSV not found: {csv_path}")
        return 0
    ...
    if len(df) == 0:
        print(f"  SKIP {stock} {year} — no rows for this year")
        return 0
    ...
    print(f"  {stock} {year} tb{max_tb_gap}: {len(out)} signals -> {os.path.basename(out_file)}")
    return len(out)


print(f"Generating {len(STOCKS)} stocks × {len(YEARS)} years × tb{TB}...\n")
total = 0
for year in YEARS:
    print(f"--- {year} ---")
    for stock in STOCKS:
        total += export_signals(stock, year, TB)
print(f"\nDone. Total signals generated: {total}")
