...
SL (0.3-1.5) x TP (0.3-2.0) grid, all 30 stocks, 2022-2025
...
TP_VALS = [round(x,1) for x in np.arange(0.3, 2.1, 0.2)]
...
print('Running tight SL x TP grid sweep...')
...
for v,s,t in flat[:3]:
    ...
    sh=sharpe(all_t)
    print(f'  SL={s}x TP={t}x  PF={v:.3f}  Sharpe={sh:.3f}  N={len(all_t)}')
...
tp_labels=[f'{x}' for x in TP_VALS]
plt.style.use('dark_background')
fig,ax=plt.subplots(figsize=(13,7))
fig.patch.set_facecolor('#0d1117'); ax.set_facecolor('#0d1117')
im=ax.imshow(grid_flipped,cmap='RdYlGn',vmin=0.7,vmax=1.3,aspect='auto')
ax.set_xticks(range(len(TP_VALS))); ax.set_xticklabels(tp_labels,color='#aaa')
...
plt.tight_layout()
out=f'{OUT_DIR}/abc_short_sl_tp_heatmap_tight.png'
