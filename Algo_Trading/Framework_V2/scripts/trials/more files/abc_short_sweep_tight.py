"""
Trading ABC — SHORT side tight range sweep
SL (0.3-1.5) x TGT (0.3-2.0) grid, all 30 stocks, 2022-2025
...
SL_VALS  = [round(x,1) for x in np.arange(0.3, 1.6, 0.2)]
TGT_VALS = [round(x,1) for x in np.arange(0.3, 2.1, 0.2)]
...
print('Running tight SL x TGT grid sweep...')
...
# Top 3 combos
flat=[(grid[si,ti], SL_VALS[si], TGT_VALS[ti])
      for si in range(len(SL_VALS)) for ti in range(len(TGT_VALS))]
flat.sort(reverse=True)
print(f'\nTop 3 combos:')
for v,s,t in flat[:3]:
    all_t=[]
    for df,sigs in all_data: all_t.extend(simulate(df,sigs,s,t))
    sh=sharpe(all_t)
    print(f'  SL={s}x TGT={t}x  PF={v:.3f}  Sharpe={sh:.3f}  N={len(all_t)}')
    print(f"  {'Year':<6} {'N':>5}  {'PF':>6}  {'Sharpe':>7}  {'WR%':>6}")
    for yr in [2022,2023,2024,2025]:
        g=[x for x in all_t if x['year']==yr]
        if not g: continue
        gp=sum(x['pnl'] for x in g if x['pnl']>0)
        gl=sum(-x['pnl'] for x in g if x['pnl']<0)
        wr=sum(1 for x in g if x['pnl']>0)/len(g)*100
        print(f"    {yr}  {len(g):>5}  {gp/gl if gl>0 else 0:>6.3f}  {sharpe(g):>7.3f}  {wr:>5.1f}%")

# Heatmap
grid_flipped=np.flipud(grid); sl_labels=[f'{x}' for x in SL_VALS[::-1]]
tgt_labels=[f'{x}' for x in TGT_VALS]
plt.style.use('dark_background')
fig,ax=plt.subplots(figsize=(13,7))
fig.patch.set_facecolor('#0d1117'); ax.set_facecolor('#0d1117')
im=ax.imshow(grid_flipped,cmap='RdYlGn',vmin=0.7,vmax=1.3,aspect='auto')
ax.set_xticks(range(len(TGT_VALS))); ax.set_xticklabels(tgt_labels,color='#aaa')
...
ax.set_title(f'PF Heatmap — Trading ABC SHORT Tight Range\n30 Stocks | 2022-2025 | N={total}',
...
plt.tight_layout()
out=f'{OUT_DIR}/abc_short_sl_tgt_heatmap_tight.png'
