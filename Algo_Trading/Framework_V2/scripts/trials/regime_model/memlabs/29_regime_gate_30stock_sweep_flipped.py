"""
Step 29 — same as step 28, EXCEPT the gating direction is flipped:
signal=+1 (predict UP) -> gate SHORT trades (instead of LONG)
signal=-1 (predict DOWN) -> gate LONG trades (instead of SHORT)

Sanity/robustness check: if this flipped mapping outperforms step 28's
"intuitive" mapping, that's either a sign-convention bug worth re-checking,
or (more likely, per discussion) a genuine strategy-specific quirk -- e.g.
MA-rejection SHORT (a pullback-to-MA setup) doing better IN an uptrend than
against one. Everything else (trade logs, models, Train/Test split, cost
formulas) is identical to step 28 -- only the two `== -1` / `== 1` gate
...
SHORT_SL_MULT, SHORT_TP_MULT = 2.0, 4.5
LONG_SL_MULT, LONG_TP_MULT = 2.0, 5.5
EOD_HOUR = 15
...
def process_symbol(symbol):
    ...
    for model_name, features in [
        ...
        for split in ['train', 'test']:
            ...
            # FLIPPED: signal==+1 -> SHORT, signal==-1 -> LONG
            ...
            rows.append(summarize(s_split[s_split['signal'] == 1], symbol, model_name, split, 'SHORT gated (flipped)'))
            rows.append(summarize(l_split, symbol, model_name, split, 'LONG baseline'))
            rows.append(summarize(l_split[l_split['signal'] == -1], symbol, model_name, split, 'LONG gated (flipped)'))
            rows.append(summarize(combined_baseline, symbol, model_name, split, 'COMBINED baseline'))
            rows.append(summarize(combined_gated, symbol, model_name, split, 'COMBINED gated (flipped)'))
...
def main():
    ...
    out_path = OUT_DIR / '29_regime_gate_30stock_sweep_flipped_results.csv'
    result.to_csv(out_path, index=False)
    print()
    print(f'Saved {len(result)} rows ({result["symbol"].nunique()} symbols) to {out_path}')

    check = result[(result['split'] == 'test') & (result['bucket'] == 'COMBINED gated (flipped)')]
