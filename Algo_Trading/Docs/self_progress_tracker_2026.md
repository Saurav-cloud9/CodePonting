...
**TOMORROW'S TODO**
- work with Gemini + Chatgpt to develop a Mock Data Generator script for backtesting.
- Check how Redis can be used for storing live trade data for better analysis.
- if time permits check with Claude regarding mega_backtest_48M_30S_v1_4_5 analysis, this is the one with all extreme configs and no anti-chasing filter. This one can be analysed for no filter performance.
- Based on our current backtesting, need to either revisit strategy or the GSS route based on how the best and worst months work.
